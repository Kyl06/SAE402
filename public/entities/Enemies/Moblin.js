// Ennemi Moblin : IA patrouille/poursuite, host-only
...
export class Moblin extends Entity {
    ...
    update(delta) {
        if (this.painState) { this.handlePain(delta); }
        else { this.think(); this.move(delta); }
        super.update(delta);
    }

    // Cible le joueur le plus proche dans un rayon de 200px
    think() {
    ...
    // Test de collision prospectif pour l'IA
    isBlocked(dirX, dirY) {
        const step = 10;
        const [testX, testY] = [this.x + dirX * step, this.y + dirY * step];
        for (let e of window.game.engine.entities) {
    ...
    // Poursuite avec évitement d'obstacles
    chaseTarget() {
        ...
        if (!this.isBlocked(primX, primY)) {
            this.velX = primX * this.chaseSpeed; this.velY = primY * this.chaseSpeed;
        } else if (!this.isBlocked(secX, secY)) {
            this.velX = secX * this.chaseSpeed; this.velY = secY * this.chaseSpeed;
        } else { this.velX = 0; this.velY = 0; }
    ...
    takeDamage(amount, direction) {
        if (typeof amount === "string") { direction = amount; amount = 1; }
    ...
    // Loot pondéré : coeur si joueur blessé, sinon émeraude
    spawnLoot() {
        const engine = window.game.engine;
